...
```
Prompt Vault has one purpose: to export the conversation currently open on a supported AI
chat site to a local file (Markdown, PDF, JSON, or HTML) that the user downloads to their
own device.
```
