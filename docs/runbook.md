...
| Release a build | Bump `package.json` (+ `npm install --package-lock-only`, `manifest.config.ts` reads the same version) on a branch and merge to `main`. `.github/workflows/release.yml` compares against the pre-push tip (`github.event.before`), re-runs `check:versions`/`lint`/`typecheck`/`test`, then `npm run package`, and publishes the zip as GitHub Release `v<version>` with generated notes (marked Latest only when it is the highest tag). Merges that leave the version alone publish nothing; an existing tag/release for that version is a no-op. A hand-cut tag therefore suppresses that version's release for good — recover with **Actions → Release → Run workflow** on `main`, passing `version=<the package.json version>`; dispatch is refused from any other ref. The dispatch publishes despite the existing tag, but refuses when that tag points at a different commit (repoint or delete the tag first), when the release itself already exists, or when the tag or release state cannot be read at all. Web Store upload stays manual — download the release asset |
| Cut a release | Run the project-local `release` skill (`.claude/skills/release/SKILL.md`) — it owns the order and the gates around the commands in this table (scope → bump → gate → mandatory pre-release checks → merge → confirm published → manual store upload) |
